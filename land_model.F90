...
subroutine land_diag_init(clonb, clatb, clon, clat, time, &
 ...
 ! number of x- and y-points.
  ...
  do n=1,gex_get_n_ex(MODEL_ATMOS,MODEL_LAND)
      id_gex_atm2lnd_diag(n) = register_tiled_diag_field ( module_name, trim(gex_get_property(MODEL_ATMOS,MODEL_LAND,n,gex_name))//'_gex_atm2lnd', axes, time, &
  ...
  do n=1,gex_get_n_ex(MODEL_LAND,MODEL_ATMOS)
      id_gex_lnd2atm_diag(n) = register_tiled_diag_field ( module_name, trim(gex_get_property(MODEL_LAND,MODEL_ATMOS,n,gex_name))//'_gex_lnd2atm', axes, time, &
