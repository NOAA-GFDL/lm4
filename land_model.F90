...
!$OMP                                  private(i,j,k,ce,tile,ISa_dn_dir,ISa_dn_dif,n_cohorts,snow_depth,snow_area)
  do l = lnd%ls, lnd%le
     ...
     do while (loop_over_tiles(ce,tile,k=k))
        ...
        call check_temp_range(land2cplr%t_ca(l,k),'tcheck15','land2cplr')
        ...
        call check_temp_range(land2cplr%t_ca(l,k),'tcheck16','land2cplr')
...
subroutine update_land_model_fast_0d ( tile, l,itile, N, land2cplr, &
  ...
  real :: ALUD(3*N+3,3*N+3) ! LU-decomposed A
  real :: X0(3*N+3),X1(3*N+3),X2(3*N+3) ! copy of the above, only for debugging
  integer :: indx(3*N+3) ! permutation vector
  ! indices of variables and equations for implicit time stepping solution :
  integer :: iqc, iFog, iTc, iTv, iwl, iwf
  ...
  ! time step:
  real :: delta_qc, delta_Tc, delta_Tv(N), delta_wl(N), delta_ws(N), delta_Tg, delta_psig, delta_co2, delta_fog
  ...
  call check_temp_range(land2cplr%t_ca(l,itile),   'tcheck17', 'land2cplr%t_ca')
...
!   endif
  ...
  call check_temp_range(tile%cana%T, 'tcheck17.4', 'tile%cana%T')
  cana_T   = tile%cana%T
...
!   fswg     = SUM(tile%Sg_dir*ISa_dn_dir + tile%Sg_dif*ISa_dn_dif)
!   ! ----- EZSNOW : additional 2 vars needed for snicar albedo option:
!   ! EZSNOW : move this before soil step 1
!   fswg_dir = tile%Sg_dir * ISa_dn_dir
!   fswg_dif = tile%Sg_dif * ISa_dn_dif
!   ! -----
  vegn_fsw = 0
  ...
  do lw_step = 1,2
     ...
     do canopy_water_step = 1,max_canopy_water_steps
        if(is_watch_point()) then
           ...
           __DEBUG1__(DHvDTv)
           __DEBUG1__(DHvDTc)
           call dpri("Ev0",Et0+Eli0+Esi0); write(*,*)
           call dpri("DEvDTv",DEtDTv+DEliDTv+DEsiDTv); write(*,*)
           call dpri("DEvDqc",DEtDqc+DEliDqc+DEsiDqc); write(*,*)
           call dpri("DEvDwl",DEtDwl+DEliDwl+DEsiDwl); write(*,*)
           call dpri("DEvDwf",DEtDwf+DEliDwf+DEsiDwf); write(*,*)
           __DEBUG1__(Et0)
           __DEBUG1__(DEtDTv)
           __DEBUG1__(DEtDqc)
           __DEBUG1__(DEtDwl)
      ...
      !     __DEBUG3__(flwg0, DflwgDTg, DflwgDTv(1))
        ...
        iqc=1;  iTc=2;  iFog=3;  iTv=4;  iwl=iTv+N;  iwf=iwl+N

        fog_step=1; fog_form = 0.0
        do ! fog_step loop
           if(is_watch_point()) then
              __DEBUG1__(fog_step)
              __DEBUG3__(tile%cana%fog, fog_form, fog_diss)
         ...
         ! [X.1.1] equation of canopy air mass balance
           ...
           enddo
           B0(iqc)  = sum(f(:)*(Esi0(:)+Eli0(:)+Et0(:)))+Eg0-Ea0-fc0
           B1(iqc)  = DEgDTg
           B2(iqc)  = DEgDpsig

         ! Fog mass balance
           A(iFog,iFog) = 1.0/delta_time
           A(iFog,iqc ) = -DfcDqc
           A(iFog,iTc)  = -DfcDTc
           B0(iFog) = fc0
           B1(iFog) = 0
...
!              enddo
         ...
         !   grnd_flux  = G0    + DGDTg*delta_Tg
           ...
           if(is_watch_point())then
              ...
              __DEBUG1__(Mg_imp)
              write(*,*)'#### tendencies of prognostic variables'
              __DEBUG4__(delta_qc, cana_q, cana_q+delta_qc, cana_qsat+DqsatDTc*delta_Tc)
              __DEBUG3__(delta_Tc, cana_T, cana_T+delta_Tc )
              __DEBUG3__(delta_Tg, grnd_T, grnd_T+delta_Tg )
              call dpri('delta_fog',delta_fog); call dpri('fog',tile%cana%fog); call dpri('fog+delta_fog',tile%cana%fog+delta_fog); write(*,*)
              ...
              __DEBUG4__(tile%lwup,ILa_dn,vegn_flw,flwg)
  ...
  call check_temp_range(tile%cana%T, 'tcheck17.5', 'tile%cana%T')
  tile%cana%T = tile%cana%T + delta_Tc
  call check_temp_range(tile%cana%T, 'tcheck17.6', 'tile%cana%T')
...
! ALSO, DIAGNOSTICS IN COMPONENT MODULES SHOULD _FOLLOW_ THIS ADJUSTMENT******
  ...
  if (associated(tile%vegn)) then
     ...
     call check_temp_range(tile%cana%T, 'tcheck17.7', 'tile%cana%T')
     call vegn_step_3 (tile%vegn, tile%soil, tile%cana%T, precip_l+precip_s, &
          ndep_nit, ndep_amm, ndep_org, vegn_fco2, tile%diag)
     call check_temp_range(tile%cana%T, 'tcheck17.8', 'tile%cana%T')
  ...
  call check_temp_range(tile%cana%T, 'tcheck17.9', 'tile%cana%T')
  ...
  call check_temp_range(land2cplr%t_ca(l,itile), 'tcheck18', 'land2cplr')
  call check_temp_range(tile%cana%T, 'tcheck18.1', 'tile%cana%T')
  call update_land_bc_fast (tile, N, l, itile, land2cplr)
  call check_temp_range(land2cplr%t_ca(l,itile), 'tcheck19', 'land2cplr')
...
subroutine update_land_bc_fast (tile, N, l,k, land2cplr, is_init)
  ...
  if (associated(tile%cana)) call check_temp_range(tile%cana%T,'tcheck25','tile%cana%T')
...
! ======= EZSNOW end - updated snow albedo
  ...
  if (associated(tile%vegn)) then
     ...
     call check_temp_range(land2cplr%t_ca(l,k),'tcheck20','tile%vegn')
     ...
     call check_temp_range(land2cplr%t_ca(l,k),'tcheck21','tile%vegn')
     ...
     call check_temp_range(land2cplr%t_ca(l,k),'tcheck22','tile%vegn')
  ...
  call check_temp_range(land2cplr%t_ca(l,k),'tcheck23','land2cplr%t_ca')
  ...
  call check_temp_range(land2cplr%t_ca(l,k),'tcheck24','land2cplr%t_ca')
  do tr = 1,ntcana
