  ...
  call read_snicar_optics_data()
   ...
   do il=1, NL ! SNICAR STARTS FROM THE BOTTOM
      ...
      ! init conc values - let's start with clean snow
         ...
         ! compute LAP concentrations for each specie and each snow layer,
         ! in [kg/kg] as required for SNICAR routine
         ! cap concentration to 1000 ppb [for BC,OC]           -> 1E-6 kg/kg
   ...
   ! write(*,*) "snicar NIR diffuse absorbed flux fractions = ",albsni(1,2) + sum(flx_absi_snw(1, :, 2))
         ...
         do il=1, NL
            do ib=1,NBANDS
               ...
               s%sw_frac_dir(il,ib) = flx_absd_snw(1, il, ib) / ( 1.0 - albsnd(1,ib) )
               s%sw_frac_dif(il,ib) = flx_absi_snw(1, il, ib) / ( 1.0 - albsni(1,ib) )
            enddo
         enddo
