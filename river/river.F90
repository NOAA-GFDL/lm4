...
!#####################################################################
  ...
  subroutine save_river_restart(timestamp)
    ...
    integer, dimension(:,:,:), allocatable :: buffer3d,buffer3d_1
    ...
    call register_variable_attribute(river_restart, river_res_zdim, "long_name", river_res_ydim)
    ...
    allocate(buffer3d_1(size(river%storage,1), size(river%storage,2), 1))
	 buffer3d_1(:,:,1) = river%storage
    call register_restart_field(river_restart, "storage", buffer3d_1, (/river_res_xdim, river_res_ydim, river_res_zdim,"Time"/))
    ...
    do tr = 1, num_species
		  allocate(buffer3d(size(river%storage_c,1), size(river%storage_c,2), 1))
        ...
        call register_variable_attribute(river_restart, "storage_"//trdata(tr)%name, "units", "none")
	     deallocate(buffer3d)
		  ...
		  allocate(buffer3d(size(discharge2ocean_next_c,1), size(discharge2ocean_next_c,2), 1))
