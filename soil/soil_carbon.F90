...
type litterCohort
    !Carbon pools
    real,dimension(N_C_TYPES)::litterC    = 0.0 !Carbon substrate. Relative amounts of C species are important for resp rate
    real,dimension(N_C_TYPES)::protectedC = 0.0 !Aggregate and mineral complex C
...
!    endif
    ...
    if(liquid_water > 0.0) then
        ...
        deposited_C=DOC_deposition_rate*max(pool%dissolved_carbon,(/0.0,0.0,0.0/))*liquid_frac*dt
        where(deposited_C > max(pool%dissolved_carbon,(/0.0,0.0,0.0/)))
...
subroutine add_carbon_to_cohorts(pool,litterC, protectedC, livingMicrobeC, CO2)
    ...
    call check_var_range(litterCval,0.0,HUGE(1.0),'add_carbon_to_cohorts','litterC',WARNING)
    call check_var_range(protectedCval,0.0,HUGE(1.0),'add_carbon_to_cohorts','protectedC',WARNING)
    call check_var_range(livingMicrobeCval,0.0,HUGE(1.0),'add_carbon_to_cohorts','livingMicrobeCval',WARNING)
    call check_var_range(CO2val,0.0,HUGE(1.0),'add_carbon_to_cohorts','CO2',WARNING)
    ...
    pool%carbon_in(:) = pool%carbon_in(:) + LitterCval(:)
    ! should we also add protectedCval, livingMicrobeCval, CO2val?

end subroutine add_carbon_to_cohorts
...
subroutine tracer_advection(tracer_mass,flow,div,dz,wl,del_tracer,divergence_loss)
    ...
    if (is_watch_point()) then
       ...
       do i = 1,nlayers
          ...
          if (i<=size(flow)) call dpri(' flow=',flow(i))
          if (i<=size(div))  call dpri(' div=',div(i))
...
!    end do
    ...
    if (is_watch_point()) then
       ...
       do i = 1,nlayers
          write(*,'(a,i2.2)',advance='NO') 'level=', i
          call dpri(' tracer_concentration=',tracer_concentration(i))
          call dpri(' aaa=',aaa(i))
          call dpri(' bbb=',bbb(i))
          call dpri(' ccc=',ccc(i))
          call dpri(' ddd=',ddd(i))
          call dpri(' flow_eff=',flow_eff(i))
          if (i<nlayers) call dpri(' u_plus=',u_plus(i))
          call dpri(' u_minus=',u_minus(i))
          if (i<=size(del_tracer)) call dpri(' del_tracer=',del_tracer(i))
          if (i<=size(dz)) call dpri(' dz=',dz(i))
          write(*,*)
       enddo
    endif
