...
use vegn_data_mod, only : spdata, nspecies, agf_bs, do_ppa, is_cropland, &
      SP_C4GRASS, SP_C3GRASS, SP_TEMPDEC, SP_TROPICAL, SP_EVERGR, &
      LU_PAST, LU_NTRL, LU_SCND, LU_RANGE, FORM_GRASS, FORM_WOODY
...
!!! dsward_crownfires
    call vegn_fire_intensity(vegn,soilc,ROS_surface,ROS,theta,theta_extinction,crown_scorch_frac, &
...
!!! dsward_crownfires
subroutine vegn_fire_intensity(vegn,soilc,ROS_surface,ROS,theta,theta_extinction,crown_scorch_frac, &
                fire_intensity,LB,fire_dur,fire_line_length)
    type(vegn_tile_type), intent(inout) :: vegn
    class(soil_BGC_t),    intent(in)    :: soilc
  ...
  !!! Note the factor of 0.45 which is intended to convert kg(C)/m2 to kg(DM)/m2
    ! AP Adjusted to add max of (0 | litter) to avoid neg values of litter
...
subroutine vegn_burn_ppa(tile)
  ...
  real :: burned_C, burned_CC, burned_N
  real :: burned_SP(0:nspecies-1) ! burned biomass per species, kgC/m2
  real :: burned_frac(N_LITTER_POOLS)
...
subroutine vegn_burn_lm3(vegn,soilc,tile_area_m2)
  ...
  burned_litt_C(i) = burned_C
  burned_litt_N(i) = burned_N
...
subroutine fire_transitions_0D(tiles, land_area, l)
  ...
  do while (loop_over_tiles(ts,tile,k=k))
     ...
     if ( do_fire_tiling &
         ...
         .AND. (.not.is_cropland(tile%vegn%landuse)) &
...
function burns_as_agri(tile) result(answer)
  ...
  answer =   is_cropland(tile%vegn%landuse) &
