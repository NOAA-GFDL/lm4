...
use vegn_data_mod, only : spdata, nspecies, do_ppa, track_vegn_nitrogen, &
     ...
     ALLOM_EW, ALLOM_EW1, ALLOM_HML, is_cropland, &
...
integer :: &
    id_mrz_scav_alloc, id_mrz_scav_immob, &
    ...
    id_rhiz_exud_mgain, id_exudate, id_mrz_scav_N_uptake,&
...
subroutine vegn_dynamics_init(id_ug, time, delta_time)
  ...
  id_exudate = register_cohort_diag_field ( diag_mod_name, 'exudate', (/id_ug/), &
       time, 'total rate of plant carbon loss to symbionts and exudates', 'kg C/(m2 year)', &
       missing_value=-100.0)
  ...
  id_total_plant_N_uptake = register_cohort_diag_field ( diag_mod_name, 'plant_N_uptake',  &
       ...
       missing_value=-100.0 )
  id_rhiz_exud_mgain = register_cohort_diag_field ( diag_mod_name, 'rhiz_exud_mgain',  &
       (/id_ug/), time, 'Extra N acquisition per unit rhiz C exudation', 'kg N/(m2 year)/kgC', &
       missing_value=-100.0 )
  id_scav_plant_N_uptake = register_cohort_diag_field ( diag_mod_name, 'plant_scav_N_uptake',  &
       (/id_ug/), time, 'Plant N uptake from scavenger mycorrhizae', 'kg N/m2/year', missing_value=-1.0 )
  id_mine_plant_N_uptake = register_cohort_diag_field ( diag_mod_name, 'plant_mine_N_uptake',  &
...
subroutine  update_mycorrhizae(cc, soilT, &
  C_allocation_to_N_acq, scav_N_uptake, mine_N_uptake, mine_C_uptake, root_N_uptake,&
  scav_efficiency, mine_efficiency,&
  scav_C_alloc, mine_C_alloc, Nfix_C_alloc,&
  ...
  real, intent(in) :: scav_N_uptake, mine_N_uptake, mine_C_uptake, root_N_uptake  ! N uptake per individual (kgN/m2)
  real, intent(in) :: C_allocation_to_N_acq
  real, intent(in) :: scav_efficiency, mine_efficiency         ! N uptake/myc biomass -- Only used if myc biomass is zero

  real, intent(out):: scav_C_alloc, mine_C_alloc, Nfix_C_alloc ! Allocation per individual (kgC/m2)
  ...
  if (.not.track_vegn_nitrogen) then
     ...
     root_exudate_C = C_allocation_to_N_acq*dt_fast_yr - scav_C_alloc - mine_C_alloc - Nfix_C_alloc + reservoir_C_leakage
     root_exudate_N = 0.0
     scav_N_to_plant = 0.0
     mine_N_to_plant = 0.0
     fix_N_to_plant = 0.0
  ...
  if(is_watch_point()) then
     ...
     __DEBUG4__(scav_N_uptake, mine_N_uptake, mine_C_uptake, root_N_uptake)
     __DEBUG2__(scav_efficiency, mine_efficiency)
  ...
  cc%scav_N_reservoir = cc%scav_N_reservoir-cc%scav_N


  mine_growth = sp%myc_growth_rate*cc%mine_C_reservoir/(cc%mine_C_reservoir+sp%kM_myc_growth)*myc_mine_C_efficiency*dt_fast_yr
  maint_resp=min(cc%mine_C/mycorrhizal_turnover_time*(1.0-et_myc)*dt_fast_yr,mine_growth)
  if (mine_growth-maint_resp>sp%c2n_mycorrhizae*cc%mine_N_reservoir*0.9) then
    ! Not enough nitrogen to support growth. Limit to available N, and leave a little bit left over for plant
    mine_growth=sp%c2n_mycorrhizae*cc%mine_N_reservoir*0.9+maint_resp
  endif
  ...
  reservoir_C_leakage = reservoir_C_leakage + (cc%scav_C_reservoir + cc%mine_C_reservoir + cc%nfix_C_reservoir)*dt_fast_yr*365
  cc%scav_C_reservoir = cc%scav_C_reservoir - cc%scav_C_reservoir*dt_fast_yr*365
  cc%mine_C_reservoir = cc%mine_C_reservoir - cc%mine_C_reservoir*dt_fast_yr*365
  cc%nfix_C_reservoir = cc%nfix_C_reservoir - cc%nfix_C_reservoir*dt_fast_yr*365

  if(abs(cc%mine_N_reservoir)<1e-20) cc%mine_N_reservoir = 0.0
  if(abs(cc%mine_C_reservoir)<1e-20) cc%mine_C_reservoir = 0.0
  if(abs(cc%scav_N_reservoir)<1e-20) cc%scav_N_reservoir = 0.0
  if(abs(cc%scav_C_reservoir)<1e-20) cc%scav_C_reservoir = 0.0
  call check_var_range(cc%nfix_C_reservoir, -1e-10,HUGE(0.0),'update_mycorrhizae #3','cc%nfix_C_reservoir',FATAL)
  call check_var_range(cc%nfix_N_reservoir, -1e-10,HUGE(0.0),'update_mycorrhizae #3','cc%nfix_N_reservoir',FATAL)
  ...
  if (sp%do_N_scavenging_strategy) then
     scav_N_to_plant = cc%scav_N_reservoir*sp%myc_N_to_plant_rate*dt_fast_yr
     if (cc%scav_C > 0) then
  ...
  cc%scav_N_reservoir = cc%scav_N_reservoir - scav_N_to_plant

  ! Mycorrhizal N mining (ECM-style)
  if (sp%do_N_mining_strategy) then
     mine_N_to_plant = cc%mine_N_reservoir*sp%myc_N_to_plant_rate*dt_fast_yr
     if(cc%mine_C>0) then
  ...
  cc%mine_N_reservoir = cc%mine_N_reservoir - mine_N_to_plant
  ...
  if (sp%do_N_fixation_strategy) then
     fix_N_to_plant = cc%nfix_N_reservoir*sp%myc_N_to_plant_rate*dt_fast_yr
     if(cc%nfix_C>0) then
  ...
  cc%nfix_N_reservoir = cc%nfix_N_reservoir - fix_N_to_plant
  ...
  if (scav_mgain+Nfix_mgain+mine_mgain+rhiz_exud_mgain>0) then
     ...
     mine_exud_frac = 0.3*0.7
     Nfix_exud_frac = 0.3*0.7
     rhiz_exud_frac = 0.3
  endif
...
subroutine vegn_carbon_int_lm3(vegn, soil, soilc, soilt, theta, diag)
  ...
  real, dimension(vegn%n_cohorts) :: scav_N_uptake, mine_N_uptake, mine_C_uptake, root_active_N_uptake
  integer :: i, l, N
  ...
  do i = 1, vegn%n_cohorts
     ...
     C_alloc_to_N_acq(i) = max(npp(i),0.0)*root_exudate_frac
     cc%carbon_gain = cc%carbon_gain + (npp(i)-C_alloc_to_N_acq(i))*dt_fast_yr
...
!     if (cc%bwood_gain < 0.0) cc%bwood_gain=0.0; ! potential non-conservation ?
     ...
     call update_mycorrhizae(cc, soilt, &
          C_allocation_to_N_acq=C_alloc_to_N_acq(i),&
          scav_N_uptake=scav_N_uptake(i),mine_C_uptake=mine_C_uptake(i),mine_N_uptake=mine_N_uptake(i),&
          root_N_uptake=root_active_N_uptake(i),&
          scav_efficiency=scav_efficiency,mine_efficiency=mine_efficiency,&
          scav_C_alloc=scav_C_alloc(i),mine_C_alloc=mine_C_alloc(i),&
          Nfix_C_alloc=Nfix_C_alloc(i),&
  ...
  ! FIXME: the id_npp_std dieg is incorrect
  ...
  call send_cohort_data(id_mrz_scav_N_uptake,diag,c(1:N),scav_N_uptake(1:N)/dt_fast_yr,weight=c(1:N)%nindivs, op=OP_SUM)
  call send_cohort_data(id_mrz_mine_N_uptake,diag,c(1:N),mine_N_uptake(1:N)/dt_fast_yr,weight=c(1:N)%nindivs, op=OP_SUM)
  ...
  call send_cohort_data(id_mrz_scav_N_res,          diag, c(1:N), c(1:N)%scav_N_reservoir, weight=c(1:N)%nindivs, op=OP_SUM)
  call send_cohort_data(id_mrz_scav_C_res,          diag, c(1:N), c(1:N)%scav_C_reservoir, weight=c(1:N)%nindivs, op=OP_SUM)
  call send_cohort_data(id_mrz_mine_N_res,          diag, c(1:N), c(1:N)%mine_N_reservoir, weight=c(1:N)%nindivs, op=OP_SUM)
  call send_cohort_data(id_mrz_mine_C_res,          diag, c(1:N), c(1:N)%mine_C_reservoir, weight=c(1:N)%nindivs, op=OP_SUM)
...
subroutine vegn_carbon_int_ppa (vegn, soil, soilc, tsoil, theta, diag)
  ...
  real, dimension(vegn%n_cohorts) :: &
      ...
      C_alloc_to_N_acq, scav_C_alloc, mine_C_alloc, Nfix_C_alloc, &
      scav_N_uptake, mine_N_uptake, mine_C_uptake, root_active_N_uptake, &
...
! 20170617:
     ...
     if(sp%tau_nsc_exudate>0) then
        C_alloc_to_N_acq(i) = max(cc%nsc,0.0)/sp%tau_nsc_exudate
     else
     ...
     if (sp%dynamic_root_exudation .AND. track_vegn_nitrogen) then
        ...
        C_alloc_to_N_acq(i) = C_alloc_to_N_acq(i)*cc%nitrogen_stress_smoothed
...
! ------ diagnostic section
  ...
  call send_cohort_data(id_mrz_scav_N_uptake,           diag, c(1:M), scav_N_uptake(1:M)/dt_fast_yr,             weight=c(1:M)%nindivs, op=OP_SUM)
  call send_cohort_data(id_mrz_mine_N_uptake,           diag, c(1:M), mine_N_uptake(1:M)/dt_fast_yr,             weight=c(1:M)%nindivs, op=OP_SUM)
  ...
  call send_cohort_data(id_mrz_scav_N_res,              diag, c(1:M), c(1:M)%scav_N_reservoir,                   weight=c(1:M)%nindivs, op=OP_SUM)
  call send_cohort_data(id_mrz_scav_C_res,              diag, c(1:M), c(1:M)%scav_C_reservoir,                   weight=c(1:M)%nindivs, op=OP_SUM)
  call send_cohort_data(id_mrz_mine_N_res,              diag, c(1:M), c(1:M)%mine_N_reservoir,                   weight=c(1:M)%nindivs, op=OP_SUM)
  call send_cohort_data(id_mrz_mine_C_res,              diag, c(1:M), c(1:M)%mine_C_reservoir,                   weight=c(1:M)%nindivs, op=OP_SUM)
...
subroutine vegn_phenology_lm3(vegn, soil, soilc)
  ...
  ! add litter accumulated over the cohorts
  call soilc%add_soil_matter( vegn, leaf_litter_C=leaf_litt_C, root_litter_C=root_litt_C, &
...
subroutine vegn_phenology_ppa(tile)
  ...
  enddo
  ! add litter accumulated over the cohorts
  call tile%soilc%add_soil_matter( vegn, leaf_litter_C=leaf_litt_C, leaf_litter_N=leaf_litt_N, &
...
subroutine vegn_reproduction_ppa(seed_transport_option)
  ...
  do while (loop_over_tiles(ce,tile,l,k1))
     ...
     if (is_cropland(tile%vegn%landuse) .and. .not.allow_weeds_on_crops) then
