      ...
      do tr = 1, ntcana
         ...
         do tr = 1, ntcana
            ...
            if ((trdata(tr)%nb_n_ox .gt. 0) .and. (trim(funits).ne.'mole/m2/s') .and. (id_ddep_nhx .gt. 0)) then
               call error_mesg("land_tracer_driver_init", trdata(tr)%name // 'units are not compatible with noy ddep', FATAL)
            endif
            if ((trdata(tr)%nb_n_red .gt. 0) .and. (trim(funits).ne.'mole/m2/s') .and. (id_ddep_noy .gt. 0 )) then
               call error_mesg("land_tracer_driver_init", trdata(tr)%name // 'units are not compatible with nhx ddep', FATAL)
            endif       
