...
subroutine soil_BGC_diag_init_SIMPLE(id_ug, id_zfull)
  ...
  character(*), parameter :: diag_mod_name = 'soil_BGC_SIMPLE'
  integer :: axes(2)
...
!> @brief Given soil carbon state, return total soil C (excluding surface litter)
!! @return total soil carbon, kgC/m2
...
!> @brief Given soil carbon state, return total soil nitrogen (excluding surface litter)
!! @return total soil nitrogen, kgN/m2
real function total_soil_N_SIMPLE(soilc) result(tot_N)
  class(soil_BGC_SIMPLE_t), intent(in)  :: soilc ! soil carbon data structure
  tot_N = 0.0
end function
...
subroutine step3_SIMPLE(soilc, diag)
  ...
  if (id_csoil>0)       call send_tile_data(id_csoil, soil%total_soil_C(), diag)
