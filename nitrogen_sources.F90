...
! ============================================================================
subroutine nitrogen_sources_init(time, id_lon, id_lat)
...
10   continue
 ...
 call set_default_diag_filter('soil')
  ...
  id_ndep_org = register_tiled_diag_field (diag_mod_name, 'ndep_org', (/id_lon,id_lat/), &
  ...
  id_nfert_nit = register_tiled_diag_field (diag_mod_name, 'nfert_nit', (/id_lon,id_lat/), &
       time, 'nitrate fertilization', 'kg N/(m2 year)', missing_value=-999.0)
  id_nfert_amm = register_tiled_diag_field (diag_mod_name, 'nfert_amm', (/id_lon,id_lat/), &
       time, 'ammonium fertilization', 'kg N/(m2 year)', missing_value=-999.0)
  id_nfert_org = register_tiled_diag_field (diag_mod_name, 'nfert_org', (/id_lon,id_lat/), &
  ...
  id_nmanure_nit = register_tiled_diag_field (diag_mod_name, 'nmanure_nit', (/id_lon,id_lat/), &
       time, 'manure nitrate', 'kg N/(m2 year)', missing_value=-999.0)
  id_nmanure_amm = register_tiled_diag_field (diag_mod_name, 'nmanure_amm', (/id_lon,id_lat/), &
       time, 'manure ammonium', 'kg N/(m2 year)', missing_value=-999.0)
  id_nmanure_org = register_tiled_diag_field (diag_mod_name, 'nmanure_org', (/id_lon,id_lat/), &
       time, 'manure organic nitrogen', 'kg N/(m2 year)', missing_value=-999.0)
  id_nmanure    = register_tiled_diag_field (diag_mod_name, 'nmanure', (/id_lon,id_lat/), &
       time, 'total manure nitrogen deposition', 'kg N/(m2 year)', missing_value=-999.0)

  id_nfml_nit = register_tiled_diag_field (diag_mod_name, 'nfml_nit', (/id_lon,id_lat/), &
       time, 'nitrate fertilization', 'kg N/(m2 year)', missing_value=-999.0)
  id_nfml_amm = register_tiled_diag_field (diag_mod_name, 'nfml_amm', (/id_lon,id_lat/), &
       time, 'ammonium fertilization', 'kg N/(m2 year)', missing_value=-999.0)
  id_nfml_org = register_tiled_diag_field (diag_mod_name, 'nfml_org', (/id_lon,id_lat/), &
       time, 'organic nitrogen fertilization', 'kg N/(m2 year)', missing_value=-999.0)
  id_nfml     = register_tiled_diag_field (diag_mod_name, 'nfml', (/id_lon,id_lat/), &
       time, 'total nitrogen fertilization', 'kg N/(m2 year)', missing_value=-999.0)

  id_amm_volat = register_tiled_diag_field (diag_mod_name, 'amm_volat', (/id_lon,id_lat/), &
...
subroutine update_nitrogen_sources (t0,t1)
  ...
  case (NDEP_TIMESERIES)
     if (id_ndep_nit_in>0) call time_interp_external(id_ndep_nit_in,t0,ndep_nit_buffer)
     if (id_ndep_amm_in>0) call time_interp_external(id_ndep_amm_in,t0,ndep_amm_buffer)
     if (id_ndep_org_in>0) call time_interp_external(id_ndep_org_in,t0,ndep_org_buffer)
  case default
...
! ============================================================================
subroutine nitrogen_sources(time, i, j, p_ann, precip, lu, input_nit, input_amm, input_org, diag)
  ...
  ndep_nit = ndep_nit_buffer(i,j)*(1-w) + w*ndep_nit_buffer(i,j)*p
  ndep_amm = ndep_amm_buffer(i,j)*(1-w) + w*ndep_amm_buffer(i,j)*p
  ndep_org = ndep_org_buffer(i,j)*(1-w) + w*ndep_org_buffer(i,j)*p

  ! add fertilization
  select case(lu)
  case (LU_CROP)
     fert_nit   = nfert_crop(i,j)   * fert_nit_frac
     fert_amm   = nfert_crop(i,j)   * fert_amm_frac
     fert_org   = nfert_crop(i,j)   * fert_org_frac
     manure_nit = nmanure_crop(i,j) * manure_nit_frac
     manure_amm = nmanure_crop(i,j) * manure_amm_frac
     manure_org = nmanure_crop(i,j) * manure_org_frac
     fml_nit    = nfml_nit_crop(i,j)
     fml_amm    = nfml_amm_crop(i,j)
     fml_org    = nfml_org_crop(i,j)
  case (LU_PAST)
     fert_nit   = nfert_past(i,j)   * fert_nit_frac
     fert_amm   = nfert_past(i,j)   * fert_amm_frac
     fert_org   = nfert_past(i,j)   * fert_org_frac
     manure_nit = nmanure_past(i,j) * manure_nit_frac
     manure_amm = nmanure_past(i,j) * manure_amm_frac
     manure_org = nmanure_past(i,j) * manure_org_frac
     fml_nit    = nfml_nit_past(i,j)
     fml_amm    = nfml_amm_past(i,j)
     fml_org    = nfml_org_past(i,j)
  case default
  ...
  if (is_watch_point()) then
     __DEBUG2__(i,j)
     __DEBUG3__(ndep_nit_buffer(i,j),ndep_amm_buffer(i,j),ndep_org_buffer(i,j))
     __DEBUG2__(w,p)
...
subroutine fert_data_get(fert, t0,t1,fert_rate)
  ...
  if (fert%curr_rec/=i1) then
     ...
     call horiz_interp_new(interp, fert%lonb_in, fert%latb_in, &
          lnd%lonb(lnd%is:lnd%ie+1,lnd%js:lnd%je+1), &
          lnd%latb(lnd%is:lnd%ie+1,lnd%js:lnd%je+1), &
