...
use time_interp_external2_mod, only: init_external_field, &
    & time_interp_external, time_interp_external_init
...
use vegn_data_mod, only : LU_PAST, LU_RAINF, LU_IRRIG
...
subroutine fert_data_get(fert, t0,t1,fert_rate)
  ...
  if (fert%curr_rec/=i1) then
     ...
     if (variable_exists(fert%fileobj,"tfert")) then
